            ...
            <div className="relative z-10 grid lg:grid-cols-[2fr,1.2fr] divide-y lg:divide-y-0 lg:divide-x divide-sr-border/60">
              ...
              <div className="px-6 py-6 space-y-5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="flex items-center gap-1.5 text-[9px] font-mono text-teal-600 tracking-[0.15em] uppercase">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <p className="text-[9px] font-mono uppercase tracking-wide text-slate-600">Professions Indexed</p>
                    <p className="mt-1 text-2xl font-black font-mono text-amber-300 tabular-nums">{totalProf}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-mono uppercase tracking-wide text-slate-600">Skills Mapped</p>
                    <p className="mt-1 text-2xl font-black font-mono text-cyan-300 tabular-nums">{liveNodes}+</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-mono uppercase tracking-wide text-slate-600">Operative Systems</p>
                    <p className="mt-1 text-2xl font-black font-mono text-teal-300 tabular-nums">6</p>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <p className="text-[9px] font-mono uppercase tracking-[0.2em] text-slate-600">Query Registry</p>
                  <form action="/recipes" method="GET" className="flex items-center">
                    <span className="px-3 py-2 text-[9px] font-mono text-slate-700 border border-r-0 border-sr-border bg-sr-surface shrink-0 tracking-[0.15em]">
                      QUERY
                    </span>
                    <input
                      type="search" name="q"
                    ...
                    />
                    <button type="submit" className="px-4 py-2 bg-cyan-600/20 border border-l-0 border-cyan-700/60 text-cyan-400 text-[9px] font-mono font-semibold hover:bg-cyan-600/30 transition-all shrink-0 tracking-[0.15em]">
                      SEARCH
                    </button>
                  </form>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <Link key={href} href={href}
                      className="text-[9px] font-mono text-slate-500 border border-sr-border px-3 py-1 hover:text-slate-300 hover:border-slate-600 transition-colors tracking-[0.1em]">
                      {label}
                    </Link>
                  ))}
                </div>
              ...
              <div className="px-6 py-6 flex flex-col justify-between gap-6 bg-sr-surface/40">
                ...
                <div className="flex flex-wrap gap-3">
                  <Link href="/planner"
                    className="inline-flex items-center gap-2 px-4 py-2 text-[9px] font-mono font-semibold text-cyan-300 border border-cyan-700/60 bg-cyan-600/10 hover:bg-cyan-600/20 transition-all tracking-[0.15em] uppercase">
                    Access Skill Planner →
                  </Link>
                  <Link href="/directory"
                    className="inline-flex items-center gap-2 px-4 py-2 text-[9px] font-mono text-slate-400 border border-sr-border hover:border-slate-600 hover:text-slate-200 transition-all tracking-[0.15em] uppercase">
                    Crafter Registry →
                  </Link>
                </div>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {SOON.map((s) => (
                <div key={s.href} className="border border-sr-border/50 bg-sr-surface/20 p-3 space-y-1.5 opacity-55">
                  ...
                  <p className="text-[9px] font-mono text-slate-600 leading-relaxed">{s.sub}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-mono text-slate-700 uppercase tracking-[0.15em]">Studios · Alpha Build</span>
            </div>
    ...
    <Link href={`/dispatch/${post.slug}`} className="group block border border-sr-border bg-sr-surface/30 hover:bg-sr-surface/50 transition-colors">
      <div className="px-4 py-3 border-b border-sr-border/40 bg-sr-surface/40">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <span className="text-[9px] font-mono text-slate-600 tracking-[0.2em] uppercase">Transmission · Kodaxa Dispatch</span>
          {post.ref_id && <span className="text-[9px] font-mono text-slate-700 tracking-[0.15em] uppercase">Ref: {post.ref_id}</span>}
        </div>
        <div className="flex items-center gap-4 mt-1 flex-wrap">
          <span className="text-[8px] font-mono text-slate-700">Filed: {post.published_at}</span>
          <span className="text-[8px] font-mono text-slate-700 uppercase">Division: {post.author}</span>
          <span className="text-[8px] font-mono text-slate-700">Classification: Public</span>
          {post.tag && (
      ...
      <div className="px-4 py-5 space-y-3">
        <span className={`text-[8px] font-mono uppercase tracking-[0.15em] px-2 py-0.5 border ${CATEGORY_COLORS[post.category]}`}>
        ...
        <span className="text-[9px] font-mono text-cyan-700 group-hover:text-cyan-400 transition-colors tracking-[0.15em] uppercase border-b border-cyan-900/40 group-hover:border-cyan-700/60">
          Read Full Dispatch →
        </span>
      </div>
    </Link>
  ...
  return (
    <Link href={`/dispatch/${post.slug}`} className="group flex flex-col gap-1.5 py-3 px-1 hover:bg-sr-surface/40 transition-colors">
      <div className="flex items-center gap-2 flex-wrap">
        <span className={`text-[8px] font-mono uppercase tracking-[0.15em] px-1.5 py-0.5 border ${CATEGORY_COLORS[post.category]}`}>
          {CATEGORY_LABELS[post.category]}
        </span>
        <span className="text-[8px] font-mono text-slate-700">{post.published_at} · {post.author.toUpperCase()}</span>
      </div>
...
function ToolCard({ href, codexName, realName, description, stats, accent, cta, deployStatus, division }: ToolCardProps) {
  return (
    <Link href={href} className={`group flex flex-col border border-sr-border bg-sr-surface/30 p-4 hover:bg-sr-surface/60 transition-colors ${accent}`}>
      ...
      <p className="mt-2 flex-1 text-[10px] font-mono text-sr-muted leading-relaxed">{description}</p>
        ...
        <div className="flex gap-4">
          {stats.map((s) => (
            <div key={s.label} className="flex flex-col">
              ...
              <span className="text-[8px] font-mono text-slate-700 uppercase tracking-[0.1em]">{s.label}</span>
            </div>
          ))}
        </div>
