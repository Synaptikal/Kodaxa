  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-2.5 pb-1">
        <span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[10px] font-medium ${stateColor}`}>
        ...
        <div className="px-4 py-2 flex gap-4 border-t border-slate-700/40">
          {xpCost > 0 && (
            <div className="flex flex-col">
              <span className="text-[9px] uppercase tracking-wider text-slate-500">XP Cost</span>
              <span className="text-sm font-mono text-cyan-300">{xpCost.toLocaleString()}</span>
            </div>
          )}
          {klaatu > 0 && (
            <div className="flex flex-col">
              <span className="text-[9px] uppercase tracking-wider text-slate-500">Klaatu</span>
      ...
      {node.special && (
        <div className="px-4 py-2 border-t border-slate-700/40">
          <p className="text-[9px] uppercase tracking-wider text-amber-500 mb-1">Special Ability</p>
          ...
          <p className="text-[9px] text-slate-500 mt-1">Tool: {node.special.toolType}</p>
        </div>
      )}
      ...
      {node.prerequisites.length > 0 && (
        <div className="px-4 py-2 border-t border-slate-700/40">
          <p className="text-[9px] uppercase tracking-wider text-slate-500 mb-1">Requires</p>
      ...
      {actionLabel && (
        <div className="px-4 pb-3 pt-1">
          <p className="text-[10px] text-slate-500 italic">{actionLabel}</p>
        </div>
      )}
    </div>
  );
